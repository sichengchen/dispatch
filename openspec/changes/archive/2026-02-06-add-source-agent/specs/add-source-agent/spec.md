...
The agent SHALL have access to an `evaluate_feed` tool that assesses RSS feed quality.

#### Scenario: Feed with full article content

- **WHEN** the agent calls `evaluate_feed` with a feed URL
- **AND** the feed items contain full article content (>500 characters average)
- **THEN** the tool returns `quality: "full"` with sample article lengths

#### Scenario: Feed with summaries only

- **WHEN** the agent calls `evaluate_feed` with a feed URL
- **AND** the feed items contain only summaries (<500 characters average)
- **THEN** the tool returns `quality: "summary"` with a recommendation to consider agentic extraction
